...
test_rec_file_access(void)
{
    ...
    test_file = fopen(TEST_REC, "w");
    if (test_file) {
        fprintf(test_file, "Project_Name: Test\nTest_Data: test\n");
        fclose(test_file);
    }
...
test_parse_auth_file(void)
{
    ...
    count = parse_auth_file(TEST_AUTH_FILE, entries, 10);
    CU_ASSERT(count > 0);
...
test_log_message(void)
{
    ...
    /* Test NULL parameters - should return TEST_ERR_PARAM */
    result = log_message(LOG_INFO, NULL, action, msg);
    CU_ASSERT_EQUAL(result, TEST_ERR_PARAM);
