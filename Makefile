...
# For MacOS use (CC = /usr/bin/cc && LD=/usr/bin/ld)
CC = /usr/bin/cc
LD = /usr/bin/ld
CFLAGS = -static -I/usr/local/musl/include -Ilib -std=c2x -D_POSIX_C_SOURCE=200809L -D_XOPEN_SOURCE=800 -O3 -Wall -Wextra -Werror -pedantic -Iinclude -Wshadow -Wconversion -Wstrict-prototypes -Wmissing-prototypes -Wold-style-definition -Wmissing-declarations -Wredundant-decls -Wfloat-equal -Wundef -Wbad-function-cast -Wcast-qual -Wwrite-strings -Wformat=2 -Wstrict-aliasing=2 -Wstrict-overflow=5 -Wc++-compat -Wnull-dereference -Wdouble-promotion -Wvla
LDFLAGS = -static -L/usr/local/musl/lib -lmusl -lm -Wl,--no-undefined -Wl,--warn-common -Wl, -Wl
SRC_DIR = src
BUILD_DIR = build
BIN_DIR = bin
TEST_DIR = tests
